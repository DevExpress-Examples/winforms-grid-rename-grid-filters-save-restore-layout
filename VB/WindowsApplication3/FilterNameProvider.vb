...
Namespace DXSample
    Public Class FilterNameProvider
        ...
        Private Sub OnSaveItemClick(ByVal sender As Object, ByVal e As EventArgs)
            Dim action As New FlyoutAction() With {.Caption = "Save Filter As", .Description = ""}
            Dim command1 As New FlyoutCommand() With {.Text = "Save", .Result = DialogResult.OK}
            Dim command2 As New FlyoutCommand() With {.Text = "Cancel", .Result = DialogResult.Cancel}
    ...
    Public Module Extensions
        ...
        Public Sub SaveLayoutToXmlEx(ByVal view As ColumnView, ByVal provider As FilterNameProvider, ByVal filePath As String)
            ...
            serializer.SerializeObjects(New XtraObjectInfo() { _
                New XtraObjectInfo("View", view), _
                New XtraObjectInfo("Filters", provider) _
            }, filePath, "DXSample")
        End Sub
        <System.Runtime.CompilerServices.Extension> _
        Public Sub RestoreLayoutFromXmlEx(ByVal view As ColumnView, ByVal provider As FilterNameProvider, ByVal filePath As String)
            Dim serializer As New XmlXtraSerializer()
            serializer.DeserializeObjects(New XtraObjectInfo() { _
                New XtraObjectInfo("View", view), _
                New XtraObjectInfo("Filters", provider) _
            }, filePath, "DXSample")
        End Sub
