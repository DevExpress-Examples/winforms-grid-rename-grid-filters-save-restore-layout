...
Namespace DXSample
    ...
    Public Class FilterNameProvider
        ...
        Friend Function XtraCreateGridFiltersItem(ByVal e As XtraItemEventArgs) As GridFitlerItem
            Dim fItem As GridFitlerItem = New GridFitlerItem()
        ...
        Private Sub OnViewCustomFilterDisplayText(ByVal sender As Object, ByVal e As DevExpress.XtraEditors.Controls.ConvertEditValueEventArgs)
            ...
            Dim fItem As GridFitlerItem = filters.FirstOrDefault(Function(it) Equals(it.FilterString, filterString))
        ...
        Private Sub OnSaveItemClick(ByVal sender As Object, ByVal e As EventArgs)
            Dim action As FlyoutAction = New FlyoutAction() With {.Caption = "Save Filter As", .Description = ""}
            Dim command1 As FlyoutCommand = New FlyoutCommand() With {.Text = "Save", .Result = DialogResult.OK}
            Dim command2 As FlyoutCommand = New FlyoutCommand() With {.Text = "Cancel", .Result = DialogResult.Cancel}
            action.Commands.Add(command1)
            action.Commands.Add(command2)
            Using filterNameEdit As TextEdit = New TextEdit()
                filterNameEdit.Width = 300
                If FlyoutDialog.Show(view.GridControl.FindForm(), action, filterNameEdit) = DialogResult.OK Then
                    ...
                    If fItem Is Nothing Then
                        filters.Add(New GridFitlerItem() With {.FilterString = view.ActiveFilterString, .FilterName = filterNameEdit.Text})
                    Else
    ...
    Public Module Extensions
        ...
        Public Sub SaveLayoutToXmlEx(ByVal view As ColumnView, ByVal provider As FilterNameProvider, ByVal filePath As String)
            Dim serializer As XmlXtraSerializer = New XmlXtraSerializer()
